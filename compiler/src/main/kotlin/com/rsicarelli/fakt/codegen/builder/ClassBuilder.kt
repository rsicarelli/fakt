 ...
 */
@CodeDsl
public class ClassBuilder @PublishedApi internal constructor(private val name: String) {
    @PublishedApi internal val typeParameters = mutableListOf<CodeTypeParameter>()

    @PublishedApi internal val superTypes = mutableListOf<CodeType>()

    @PublishedApi internal val modifiers = mutableSetOf<CodeModifier>()

    @PublishedApi internal val members = mutableListOf<CodeMember>()

    @PublishedApi internal var whereClause: String? = null

    @PublishedApi internal val annotations = mutableListOf<CodeAnnotation>()

    @PublishedApi internal val constructorProperties = mutableListOf<ConstructorProperty>()
     ...
     */
    public fun typeParam(name: String, vararg constraints: String, reified: Boolean = false) {
        typeParameters.add(
            CodeTypeParameter(name = name, constraints = constraints.toList(), isReified = reified)
        )
    }
    ...
    public fun implements(typeName: String, vararg typeArgs: String) {
        superTypes.add(
            if (typeArgs.isEmpty()) {
                ...
                CodeType.Generic(typeName, typeArgs.map { CodeType.Simple(it) })
            }
        )
    }
     ...
     *
     * Regions are IDE-collapsible sections (IntelliJ, Android Studio). Must be paired with
     * [endRegion].
     *
     * Example:
     * ```kotlin
     ...
     */
    public inline fun property(name: String, type: String, block: PropertyBuilder.() -> Unit = {}) {
