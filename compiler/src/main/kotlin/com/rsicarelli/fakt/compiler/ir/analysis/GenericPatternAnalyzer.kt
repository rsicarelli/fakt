...
class GenericPatternAnalyzer {
    ...
    private fun analyzeUsagePatterns(irClass: IrClass): UsageAnalysis {
        ...
        irClass.declarations.filterIsInstance<IrSimpleFunction>().forEach { function ->
            ...
            val parameterTypeHints =
                function.parameters
                    .filter { it.kind == IrParameterKind.Regular || it.kind == IrParameterKind.Context }
                    .flatMap { param -> extractTypeHints(param.type) }
            detectedTypes.addAll(parameterTypeHints)
    ...
    private fun detectTransformationPatterns(function: IrSimpleFunction): List<TransformationPattern> {
        ...
        if (function.typeParameters.size >= 2) {
            ...
            // Extract type parameter usage from parameters
            val inputTypes =
                function.parameters
                    .filter { it.kind == IrParameterKind.Regular || it.kind == IrParameterKind.Context }
                    .mapNotNull { param -> extractTypeParameterUsage(param.type, typeParams) }
