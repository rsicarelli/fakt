 ...
 *
 * **Simplified Error Reporting**: Instead of using complex FIR diagnostic factories (which may vary
 * across Kotlin versions), we use direct error reporting via session.messageCollector which is
...
internal object FirFaktErrors {
    const val FAKE_MUST_BE_INTERFACE =
        "[FAKT] @Fake can only be applied to interfaces, not classes or objects"
    ...
    const val FAKE_CLASS_MUST_BE_ABSTRACT =
        "[FAKT] @Fake class must be abstract (contain abstract or open members)"
    const val FAKE_CLASS_CANNOT_BE_SEALED = "[FAKT] @Fake class cannot be sealed"
    const val FAKE_CLASS_CANNOT_BE_FINAL =
