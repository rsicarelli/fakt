...
class UnifiedFaktIrGenerationExtension(private val sharedContext: FaktSharedContext) :
    ...
    private fun generateFromFirMetadata(moduleFragment: IrModuleFragment) {
        ...
        // FIR-phase emission (EmitPhase.FIR) already wrote every fake from FakeInterfaceChecker /
        // FakeClassChecker as each declaration was analyzed (see FirFakeEmitter). Skip IR
        // generation entirely here to avoid emitting the same fakes twice; the K2JVM driver still
        // runs this IR extension, but only the FIR-hosted path is authoritative in that mode.
        if (sharedContext.options.emitPhase == EmitPhase.FIR) {
            logger.debug("EmitPhase.FIR active - skipping IR-phase fake generation")
            return
        }
    ...
    private fun processInterfacesFromMetadata(
        ...
        for (metadata in interfaceMetadata) {
            ...
            // Build output file path using ACTUAL output directory (based on source set)
            // This must match the directory used by CodeGenerator.selectOutputDirectory()
            val actualOutputDir = selectOutputDirectory(metadata.sourceSourceSet, sourceSetContext)
            ...
            if (metadata.sourceSourceSet == null) {
                val commonTestDir = java.io.File(sourceSetContext.commonTestOutputDirectory)
                val commonTestFile = commonTestDir.resolve(packagePath).resolve("$fakeClassName.kt")
                if (commonTestFile.exists()) {
                    irCacheHits++
                    continue // Already generated by earlier compilation (e.g., metadata or jvmMain)
                }
            }
    ...
    private fun selectOutputDirectory(sourceSourceSet: String?, context: SourceSetContext): String {
        ...
        return context.commonTestOutputDirectory.replace("/commonTest/", "/$testSourceSet/")
    }
}
