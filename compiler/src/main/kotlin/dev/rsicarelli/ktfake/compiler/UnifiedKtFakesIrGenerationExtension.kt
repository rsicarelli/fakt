...
class UnifiedKtFakesIrGenerationExtension(
    ...
    private fun generateWorkingFakeImplementation(
        ...
        // **NEW: Pattern-Based Code Generation Dispatch** 🚀
        when (val pattern = analysis.genericPattern) {
            GenericPattern.NoGenerics -> {
                messageCollector?.reportInfo("KtFakes: Using NoGenerics code generation strategy")
                generateNoGenericsFake(sourceInterface, analysis, moduleFragment)
            }
            is GenericPattern.ClassLevelGenerics -> {
                messageCollector?.reportInfo("KtFakes: Using ClassLevelGenerics code generation strategy")
                generateClassLevelGenericsFake(sourceInterface, analysis, moduleFragment, pattern)
            }
            is GenericPattern.MethodLevelGenerics -> {
                messageCollector?.reportInfo("KtFakes: Using MethodLevelGenerics code generation strategy")
                generateMethodLevelGenericsFake(sourceInterface, analysis, moduleFragment, pattern)
            }
            is GenericPattern.MixedGenerics -> {
                messageCollector?.reportInfo("KtFakes: Using MixedGenerics code generation strategy")
                generateMixedGenericsFake(sourceInterface, analysis, moduleFragment, pattern)
            }
        }
    }
    
    /**
     * Generate code for interfaces with no generic parameters.
     * This is the simplest case and uses the existing generation logic.
     */
    private fun generateNoGenericsFake(
        sourceInterface: IrClass,
        analysis: InterfaceAnalysis,
        moduleFragment: IrModuleFragment
    ) {
        ...
        messageCollector?.reportInfo("KtFakes: Generated NoGenerics fake $fakeClassName at $outputFile")
    }
    
    /**
     * Generate code for interfaces with class-level generic parameters only.
     * Example: interface Repository<T, ID> { ... }
     */
    private fun generateClassLevelGenericsFake(
        sourceInterface: IrClass,
        analysis: InterfaceAnalysis,
        moduleFragment: IrModuleFragment,
        pattern: GenericPattern.ClassLevelGenerics
    ) {
        val interfaceName = analysis.interfaceName
        val fakeClassName = "Fake${interfaceName}Impl"
        val packageName = sourceInterface.packageFqName?.asString() ?: "test.sample"

        messageCollector?.reportInfo("KtFakes: Generating class-level generic fake with ${pattern.typeParameters.size} type parameters")

        // TODO: Implement specialized class-level generics generation
        // For now, fallback to the existing implementation
        generateNoGenericsFake(sourceInterface, analysis, moduleFragment)
    }
    
    /**
     * Generate code for interfaces with method-level generic parameters only.
     * Example: interface Service { fun <T> process(data: T): T }
     */
    private fun generateMethodLevelGenericsFake(
        sourceInterface: IrClass,
        analysis: InterfaceAnalysis,
        moduleFragment: IrModuleFragment,
        pattern: GenericPattern.MethodLevelGenerics
    ) {
        val interfaceName = analysis.interfaceName
        val fakeClassName = "Fake${interfaceName}Impl"
        val packageName = sourceInterface.packageFqName?.asString() ?: "test.sample"

        messageCollector?.reportInfo("KtFakes: Generating method-level generic fake with ${pattern.genericMethods.size} generic methods")

        // TODO: Implement specialized method-level generics generation
        // For now, fallback to the existing implementation
        generateNoGenericsFake(sourceInterface, analysis, moduleFragment)
    }
    
    /**
     * Generate code for interfaces with both class-level and method-level generics.
     * Example: interface Cache<K, V> { fun <R : V> computeIfAbsent(key: K): R }
     */
    private fun generateMixedGenericsFake(
        sourceInterface: IrClass,
        analysis: InterfaceAnalysis,
        moduleFragment: IrModuleFragment,
        pattern: GenericPattern.MixedGenerics
    ) {
        val interfaceName = analysis.interfaceName
        val fakeClassName = "Fake${interfaceName}Impl"
        val packageName = sourceInterface.packageFqName?.asString() ?: "test.sample"

        messageCollector?.reportInfo("KtFakes: Generating mixed generic fake with ${pattern.classTypeParameters.size} class parameters and ${pattern.genericMethods.size} generic methods")

        // TODO: Implement specialized mixed generics generation
        // For now, fallback to the existing implementation
        generateNoGenericsFake(sourceInterface, analysis, moduleFragment)
    }
    ...
    internal fun generateImplementationClass(
        ...
        return buildString {
            ...
            for (function in analysis.functions) {
                ...
                val returnTypeString = irTypeToKotlinString(function.returnType, preserveTypeParameters = false)
                val parameterTypes = function.parameters.joinToString(", ") { param ->
                    irTypeToKotlinString(param.type, preserveTypeParameters = false)
                }
            ...
            for (property in analysis.properties) {
                ...
                val returnTypeString = irTypeToKotlinString(property.type, preserveTypeParameters = false)
                appendLine(
    ...
    internal fun irTypeToKotlinString(irType: IrType): String {
        return irTypeToKotlinString(irType, preserveTypeParameters = true)
    }

    /**
     * Convert IR type to Kotlin string representation with optional type parameter handling.
     * When preserveTypeParameters = false, function-level type parameters are replaced with Any 
     * for use in class-level behavior properties.
     */
    internal fun irTypeToKotlinString(irType: IrType, preserveTypeParameters: Boolean): String {
        return when {
            ...
            irType is IrSimpleType && irType.classifier is IrTypeParameterSymbol -> {
                if (preserveTypeParameters) {
                    ...
                    val typeName = typeParam.owner.name.asString()
                    if (irType.isMarkedNullable()) "${typeName}?" else typeName
                } else {
                    // For class-level behavior properties, use Any instead of function-level type parameters
     ...
     * Provides sensible defaults for collections, common types, type parameters, and custom classes.
     */
    internal fun getDefaultValue(irType: IrType): String {
        return when {
            // Handle type parameters - for generics like <T>, we can't know the concrete type
            // so we need to return a placeholder that the caller can use based on context
            irType is IrSimpleType && irType.classifier is IrTypeParameterSymbol -> {
                val typeParam = irType.classifier as IrTypeParameterSymbol
                val typeName = typeParam.owner.name.asString()
                if (irType.isMarkedNullable()) {
                    ...
                    // For non-nullable type parameters, provide a sensible fallback
                    // This will be cast at runtime but allows compilation to succeed
                    when (typeName) {
                        "T", "K", "V", "R" -> "\"\" as Any"  // Common type parameter names
                        else -> "Unit as Any"  // Uncommon names, use Unit as safe fallback
                    }
                }
            }
