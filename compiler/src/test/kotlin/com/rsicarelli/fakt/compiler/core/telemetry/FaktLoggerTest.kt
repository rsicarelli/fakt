...
class FaktLoggerTest {
    @Test
    fun `GIVEN QUIET level WHEN logging info THEN should not log`() = runTest {
    ...
    @Test
    fun `GIVEN INFO level WHEN logging info THEN should log message`() = runTest {
        // GIVEN: Test message collector
        val messageCollector = TestMessageCollector()
    ...
    @Test
    fun `GIVEN INFO level WHEN logging debug THEN should not log`() = runTest {
    ...
    @Test
    fun `GIVEN DEBUG level WHEN logging info and debug THEN should log both`() = runTest {
    ...
    @Test
    fun `GIVEN any level WHEN logging warning THEN should always log`() = runTest {
    ...
    @Test
    fun `GIVEN any level WHEN logging error THEN should always log`() = runTest {
        // GIVEN: Test message collector at QUIET level (most restrictive)
        val messageCollector = TestMessageCollector()
        val logger = FaktLogger(messageCollector, LogLevel.QUIET)
    ...
    @Test
    fun `GIVEN null messageCollector WHEN logging THEN should not crash`() = runTest {
    ...
    @Test
    fun `GIVEN ifLevel block WHEN level matches THEN should execute block`() = runTest {
        // GIVEN: Logger at DEBUG level
        val logger = FaktLogger(null, LogLevel.DEBUG)
        var blockExecuted = false

        // WHEN: Using ifLevel with DEBUG threshold
        logger.ifLevel(LogLevel.DEBUG) { blockExecuted = true }
    ...
    @Test
    fun `GIVEN ifLevel block WHEN level does not match THEN should not execute block`() = runTest {
