...
/**
 * Tests for CodeGenerator. Validates code generation orchestration, file writing, and error
 * handling.
 *
 * Note: Complex integration tests with real IR classes are done through the samples/ directory.
 * These tests focus on unit-testable behaviors: package handling, naming conventions, message
 * collection, and code template structure.
 */
...
class CodeGeneratorTest {
    ...
    inner class CodeTemplateStructure {
        ...
        fun `GIVEN package name WHEN building package declaration THEN should format correctly`(
            packageName: String
        ) = runTest {
            // GIVEN & WHEN
        ...
        @Test
        fun `GIVEN list of imports WHEN sorting THEN should be alphabetically ordered`() = runTest {
            // GIVEN
            val imports =
                setOf("kotlin.collections.List", "kotlinx.coroutines.flow.Flow", "java.util.Date")
        ...
        override fun report(
            ...
            when (severity) {
                ...
                CompilerMessageSeverity.ERROR -> errorMessages.add(message)
                    ...
                    /* ignore other severities for this test */
                }
            }
        }
