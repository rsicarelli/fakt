...
class MetadataCacheSerializerTest {
    ...
    @Test
    fun `GIVEN ValidatedFakeInterface WHEN toSerializable THEN converts all fields correctly`() = runTest {
        // GIVEN
        val classId = ClassId.topLevel(FqName("com.example.UserService"))
        val validated = ValidatedFakeInterface(
            classId = classId,
    ...
    @Test
    fun `GIVEN nested class WHEN toSerializable THEN classIdString uses correct format`() = runTest {
    ...
    @Test
    fun `GIVEN SerializableFakeInterface WHEN toValidated THEN restores ClassId correctly`() = runTest {
        // GIVEN: ClassId.asString() format uses slashes for package
        val serializable = SerializableFakeInterface(
    ...
    @Test
    fun `GIVEN serializable with all field types WHEN toValidated THEN converts everything`() = runTest {
        // GIVEN: ClassId.asString() format uses slashes for package
        val serializable = SerializableFakeInterface(
    ...
    @Test
    fun `GIVEN same file content WHEN computeFileSignature twice THEN returns same hash`() = runTest {
        // GIVEN
    ...
    @Test
    fun `GIVEN different file content WHEN computeFileSignature THEN returns different hash`() = runTest {
        // GIVEN
    ...
    @Test
    fun `GIVEN multiple signatures WHEN computeCombinedSignature THEN produces deterministic hash`() = runTest {
        // GIVEN
    ...
    @Test
    fun `GIVEN signatures in different order WHEN computeCombinedSignature THEN produces same hash`() = runTest {
    ...
    fun `GIVEN ValidatedFakeInterface WHEN full roundtrip THEN preserves all data`() = runTest {
        ...
        val original = ValidatedFakeInterface(
            classId = classId,
